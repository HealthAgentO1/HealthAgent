...
- **Storage:** Access, refresh, and email live in **`localStorage`** (`frontend/src/api/authStorage.ts`).
- **Bootstrap:** **`AuthProvider`** (`frontend/src/context/AuthContext.tsx`) does **not** treat “access token string present” as logged-in until initialization finishes. It decodes the access JWT’s **`exp`** claim (no signature verification; see **`frontend/src/utils/jwtAccess.ts`**). If the token is **expired or malformed** and a **refresh** token exists, it **proactively** calls **`refreshAccessToken()`** (`POST /api/token/refresh/`). If refresh succeeds, the new access token is stored and the user stays signed in; if it fails or there is no refresh token, **`clearAuthSession()`** runs so the UI does not show an authenticated shell based on a dead access token alone.
- **Runtime API calls:** **`apiClient`** (`frontend/src/api/client.ts`) adds `Authorization: Bearer <access>`. On **401**, it retries the request once after a refresh (skipping retry loops for `/token/` endpoints); if refresh fails, it clears storage and sends the browser to **`/login`** (unless already there). Ongoing sessions rely on this path when the access token expires while the app is open.
...
- **Prompts** for the survey live in `frontend/src/symptomCheck/prompts/` (`followup_context.txt`, `followup_round2_context.txt`, `results_context.txt`, `price_estimate_context.txt`). The SPA loads them at build time (Vite `?raw`), sends them as `system_prompt` on each call, and sends structured `user_payload` (symptoms, insurer label, follow-up answers, and on the final call the browser **active medication regimen** — see below). **Mirrored copies** for the API live under **`api/prompts/survey/`** (same text); keep them in sync when editing prompts.
- **Production system instructions:** When **`SYMPTOM_SURVEY_USE_SERVER_PROMPTS`** is enabled (default **`True`** when `DJANGO_DEBUG` is off), **`SymptomSurveyLlmView`** ignores the client `system_prompt` and loads the matching file from **`api/prompts/survey/`** by **`phase`**. In local dev with **`DEBUG=True`**, the default is to **trust** the client-supplied `system_prompt` unless the env var forces server prompts.
- **Guards:** Survey requests are subject to **payload size caps** (`SYMPTOM_SURVEY_MAX_SYSTEM_PROMPT_CHARS`, `SYMPTOM_SURVEY_MAX_USER_PAYLOAD_BYTES` in `backend/settings.py`), **scoped rate limiting** (`symptom_survey_llm` in DRF `DEFAULT_THROTTLE_RATES`), and **structured logging** (user id, phase, exception type — not raw prompts or PHI). LLM failures return distinct HTTP statuses where possible (**503** for misconfiguration, transport, or rate limits; **502** for upstream API or invalid model output), aligned with **`SymptomChatView`** behavior.
...
- **Survey phases** (not chat-first): (1) `followup_questions` → variable `questions[]` with `input_type` for controls; (2) optional `followup_questions_round_2` if the model returns more questions; (3) `condition_assessment` → conditions, severities, and **`care_taxonomy`** (NUCC taxonomy codes for routing); (4) optional **`price_estimate_context`** for illustrative cost copy on the results step.
...
- **Reports (`/reports`):** `frontend/src/pages/ReportsPage.tsx` lists symptom sessions (sidebar labeled **Drafts — past checks**) and reads `pre_visit_report` for patient-friendly sections and PDF export. When **`post_visit_diagnosis`** is set on the session (see below), a **Post-visit Diagnosis** block appears above the pre-visit summary. **View report** on the Symptom Check results step **`refetch`es** the sessions query, navigates to **`/reports?session=<SymptomSession.public_id>`**, and passes **`state: { scrollToTop: true }`**. The Reports route **refetch**es if the `session` query id is not yet in the cache (so the new check is selected instead of snapping to the first row), shows a brief loading state while fetching, and runs **`scrollAppToTop`** (`frontend/src/utils/scrollAppToTop.ts`) in **`useLayoutEffect`** when that navigation state is present so the app shell scrolls to the top.
- **Post-visit diagnosis:** Django stores **`SymptomSession.post_visit_diagnosis`** (JSON). The SPA **`PATCH`es** **`/api/sessions/<uuid>/`** with `{ "post_visit_diagnosis": { "text", "source", "matched_condition_title" } }` after the user returns to a completed check (eligibility is tracked in **`localStorage`** so the form is not shown on the first viewing of illustrative results). The Symptom Check results step renders **`PostVisitDiagnosisSection`** (`frontend/src/symptomCheck/PostVisitDiagnosisSection.tsx`): a dropdown of LLM condition titles plus **Other** with a text field. **`GET /api/sessions/<uuid>/`** (resume) includes `post_visit_diagnosis` for hydration. **`GET /api/sessions/`** returns each row’s `post_visit_diagnosis` so the client can build **`prior_official_diagnoses`** for optional reuse (see below).
- **Prior official diagnoses on intake:** On Symptom Check **step 1**, an optional checkbox (with an info dialog) lets signed-in users send **deduplicated** labels from past checks’ **`post_visit_diagnosis.text`** to the **first** survey LLM call. When enabled, `requestFollowUpQuestions` adds **`prior_official_diagnoses`** to **`user_payload`** (see `frontend/src/symptomCheck/symptomLlmClient.ts` and `priorOfficialDiagnoses.ts`). Browser **`localStorage`** snapshot fields **`includePriorDiagnosesInLlm`** and **`priorOfficialDiagnosesSnapshot`** preserve the choice and support resuming a pending `followup` request. **`followup_context.txt`** instructs the model how to use that array.
...
- **Conversational chat** (`POST /api/symptom/chat/`): **internal / future use** — backend + tests only; the SPA does not use it. Uses `api/prompts/symptom_chat_system.txt` (JSON reply with `assistant_message`, `triage_level`, etc.).
- **Legacy router ViewSet** (`/api/symptom-sessions/`): **`SymptomSessionViewSet`** (booking mock) scopes **`get_queryset()`** and **`perform_create`** to **`request.user`**. The main product paths use **`/api/sessions/`** and **`/api/sessions/<uuid>/`** instead.
- **Provider search helper** (`GET /api/providers/`, `NPPESService.search_providers`): NUCC **codes** must be sent to the CMS NPI Registry as the **`taxonomy_code`** query parameter; **description text** uses **`taxonomy_description`** (see CMS Read API 2.1). The symptom **nearby-facilities** path uses **`taxonomy_code`** in `api/services/nppes_nearby.py`.
